...
`doctor` 不仅仅检查文件是否被修改，它进行**内容级验证**：

- **同步验证**：当文档和代码一起更新时，`doctor` 从代码 diff 中提取变更的函数/类/接口名，检查文档 diff 是否真正涉及了这些变更。仅修改空白的文档会被标记为 `TOUCHED ONLY` 而非 `SYNCED`。
- **变更摘要**：展示代码中哪些标识符发生了变化（`Changed: safeRef, getStagedFiles`）。
- **文档引用扫描**：使用 `--explain` 时，高亮文档中引用了变更代码的具体行 —— 告诉你具体哪里可能需要更新。
- **宽泛范围降级**：项目级文档（关联到 `src/**`）不会在每个 PR 中触发警告 —— 只有新增/删除文件时才告警。
